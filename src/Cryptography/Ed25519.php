...
class Ed25519
{
    ...
    public function __construct()
    {
        $this->b = 256;
        $this->q =
            '57896044618658097711785492504343953926634992332820282019728792003956564819949'; //bcsub(bcpow(2, 255),19);
        $this->l =
            '7237005577332262213973186563042994240857116359379907606001950938285454250989'; //bcadd(bcpow(2,252),27742317777372353535851937790883648493);
        $this->d =
            '-4513249062541557337682894930092624173785641285191125241628941591882900924598840740'; //bcmul(-121665,$this->inv(121666));
        $this->I =
            '19681161376707505956807079304988542015446066515923890162744021073123829784752'; //$this->expmod(2,  bcdiv((bcsub($this->q,1)),4),$this->q);
        $this->By =
            '46316835694926478169428394003475163141307993866256225615783033603165251855960'; //bcmul(4,$this->inv(5));
        $this->Bx =
            '15112221349535400772501151409588531511454012693041857206046113283949847762202'; //$this->xrecover($this->By);
        $this->B = [
    ...
    public function xrecover($y)
    {
        if ($this->gmp) {
            ...
            $xx = gmp_mul(
                gmp_sub($y2, 1),
                $this->inv(gmp_add(gmp_mul($this->d, $y2), 1))
            );
            $x = $this->expmod(
                $xx,
                gmp_div(gmp_add($this->q, 3), 8, 0),
                $this->q
            );
            ...
            $xx = bcmul(
                bcsub($y2, 1),
                $this->inv(bcadd(bcmul($this->d, $y2), 1))
            );
    ...
    public function edwards($P, $Q)
    {
        if ($this->gmp) {
            ...
            $x3 = bcmul(
                bcadd(bcmul($x1, $y2), bcmul($x2, $y1)),
                $this->inv(bcadd(1, $com))
            );
    ...
    {
        $bits = substr(
            str_pad(strrev($this->dec2bin_i($y)), $this->b, '0', STR_PAD_RIGHT),
            0,
            $this->b
        );
    ...
    public function publickey($sk)
    {
        if ($this->gmp) {
            ...
            for ($i = 3; $i < $this->b - 2; $i++) {
                $sum = gmp_add(
                    $sum,
                    gmp_mul(gmp_pow(2, $i), $this->bit($h, $i))
                );
            }
    ...
    public function signature($m, $sk, $pk)
    {
        if ($this->gmp) {
            ...
            $r = $this->Hint(
                substr($h, $this->b / 8, $this->b / 4 - $this->b / 8) . $m
            );
            $R = $this->scalarmult($this->B, $r);
            $encR = $this->encodepoint($R);
            $S = $this->pymod(
                gmp_add($r, gmp_mul($this->Hint($encR . $pk . $m), $a)),
                $this->l
            );
        } else {
            $h = $this->H($sk);
            ...
            $encR = $this->encodepoint($R);
            $S = $this->pymod(
                bcadd($r, bcmul($this->Hint($encR . $pk . $m), $a)),
                $this->l
            );
        }
    ...
    public function isoncurve($P)
    {
        if ($this->gmp) {
            ...
            return $this->pymod(
                gmp_sub(
                    gmp_sub(gmp_sub($y2, $x2), 1),
                    gmp_mul($this->d, gmp_mul($x2, $y2))
                ),
                $this->q
            ) == 0;
        } else {
    ...
    public function decodeint($s)
    {
        if ($this->gmp) {
            ...
            for ($i = 0; $i < $this->b; $i++) {
                $sum = gmp_add(
                    $sum,
                    gmp_mul(gmp_pow(2, $i), $this->bit($s, $i))
                );
            }
        } else {
            $sum = 0;
    ...
    public function checkvalid($s, $m, $pk)
    {
        ...
        return $this->scalarmult($this->B, $S) ==
            $this->edwards($R, $this->scalarmult($A, $h));
    }
